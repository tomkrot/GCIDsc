...
Before deploying Cloud Build infrastructure, verify that the service account key works by running a test export on your local workstation. This requires Python and the framework code — nothing from GCP infrastructure (Terraform, Cloud Build, etc.) is involved yet.

**Step A — Get the framework code onto your local machine:**

```bash
# If the code is in a Git repository:
git clone https://github.com/your-org/GoogleWorkspaceDsc.git
cd GoogleWorkspaceDsc

# Or if working from the zip archive:
...
This downloads all dependencies (google-api-python-client, tenacity, etc.) from PyPI and registers the `gwsdsc` command.
...
This should print a table listing all 32 resource modules.

**Step C — Create a minimal test config and run the export:**

At this point you still have the `sa-key.json` file on disk from step 3.3 (you'll upload it to Secret Manager and delete it in Phase 3). Use it directly for this one-time test.
...
If you see JSON files with your tenant's customer record and org units, the Google Workspace side is ready.  You can now proceed to deploy the Cloud Build infrastructure.
...
The smoke test venv is only needed for this one-time validation.  Cloud Build installs gwsdsc inside its own container at runtime.
